...
<div class="glass-panel" id="quizPanel" style="display: none;">
    ...
    <div class="btn-container">
        ...
        <button id="checkBtn" class="action-btn" onclick="checkAnswer()" style="background: linear-gradient(135deg, #3498db, #2980b9); box-shadow: 0 4px 15px rgba(52,152,219,0.3);">🔍 Verifica Risposta</button>
...
<script>
    ...
    function showQuestion(index) {
        ...
        // Check if question was already "checked" (we can store a flag)
        const isAlreadyChecked = window.checkedQuestions && window.checkedQuestions[index];
        ...
        q.prompt.answers.forEach((ans, i) => {
            ...
            label.className = `option-label ${isChecked && !isAlreadyChecked ? 'selected' : ''}`;
            label.id = `label_${letter}`;
            label.innerHTML = `
                <input type="${isMulti ? 'checkbox' : 'radio'}" name="option" value="${letter}" ${isChecked ? 'checked' : ''} ${isAlreadyChecked ? 'disabled' : ''}>
                ${ans.replace(/<p>|<\/p>/g, '')}
            `;
        ...
        if (isAlreadyChecked) {
            checkAnswer(true); // Re-run check visually without changing logic
        }
    ...
    function checkAnswer(isRecheck = false) {
        if (!window.checkedQuestions) window.checkedQuestions = {};
        window.checkedQuestions[currentIdx] = true;

        const q = questions[currentIdx];
        const correctArr = q.correct_response;
        const userArr = userAnswers[currentIdx] || [];
        ...
        document.querySelectorAll('.option-label').forEach(label => {
            ...
            } else if (userArr.includes(letter) && !correctArr.includes(letter)) {
        ...
        explBox.innerHTML = `<strong>Spiegazione Ufficiale AWS:</strong><br><br>${q.prompt.explanation}`;
    }
